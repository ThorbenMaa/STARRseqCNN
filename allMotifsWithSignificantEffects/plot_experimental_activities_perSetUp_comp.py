...
@click.option(
    ...
    multiple=False,
    default=False,
    help="plots are smaller if True",
)
def cli(smaller_plots, seq_file, fimo_file, activity_file, out, exp_set_ups, PWM_file, data_base_file, tomtom_qual_level, number_motifs_to_plot, outputPWM, fimo_qual_level, chunk_size ):
    ...
    #initialize table with p-value of motif specificity
    motif_list = []
    p_val_ctrl = []
    p_val_treatment = []
    p_val_difference = []
    ...
    for motivsToCheck in np.array_split(all_motivsToCheck, chunk_size):
        ...
        if smaller_plots == False:
            ...
            fig, axs = plt.subplots(nrows=len(motivsToCheck), ncols=3+number_motifs_to_plot*2+1, sharex=False, sharey=False, figsize=((3+number_motifs_to_plot*2+1)*3 ,2*len(motivsToCheck)))
        # figure font sizes
        SMALL_SIZE = 4
        ...
        for i in range (0, len(motivsToCheck), 1):
            ...
            # fill motif list        
            motif_list.append(str(motivsToCheck[i]))
            ...
            # write statistics to extra list
            p_val_ctrl.append(stats.kruskal(data[0], data[1])[1])
            p_val_treatment.append(stats.kruskal(data[2], data[3])[1])
            p_val_difference.append(stats.kruskal(data[4], data[5])[1])
            have_motifs.append(data[0].shape[0])
            have_not_motif.append(data[1].shape[0])
    ...
    # write file with motif specificity statistics
    df_stats = pd.DataFrame()
    df_stats["MOTIF ID"] = motif_list
    df_stats["p-val control"] = p_val_ctrl
    df_stats["p-val treatment"] = p_val_treatment
    df_stats["p-val difference"] = p_val_difference
    df_stats["n(have motif)"] = have_motifs
    df_stats["n(has not motif)"] = have_not_motif
    df_stats.to_csv(out+"motif_specificity_stats.tsv", sep='\t')
