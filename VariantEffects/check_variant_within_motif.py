...
def cli(fimo_file, variants_file, out, q_thres, p_thres, p_val_source):

    #import labels; abs diff activities of seq with ID1 - ID2
    df_activities=pd.read_csv(variants_file, sep=",", low_memory=False)
    print("number of sig haplo effects :", df_activities.drop_duplicates(subset=["ID1","ID2"]).shape[0])
    ...
    if p_val_source == "MPRAlm":
        ...
        df_out1["p-value haplotype effect"] = df_fimo_hits1["pvalue"]
    else:
    ...
    if p_val_source == "MPRAlm":
        ...
        df_out2["p-value haplotype effect"] = df_fimo_hits2["pvalue"]
    else:
        raise ValueError('please specify p-value source')
